...
Each column can take a renderExpandableColumn function which will render a row underneath the column in question which wil allow the user to show
an expandable section below when toggled.

Note: You can have more than one renderExpandableColumn, they're handled independently.
Note: If you have sorting enabled and expand the expanded section to sort alongside the table data, you'll need to provide a `customRowKey` to the table
to allow the expanding section to sort alongside the data.
